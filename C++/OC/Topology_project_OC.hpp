...
vec dcda_harm(vec& lambda, vec& T, vec& a, mat& k, int N, double penal){
	...
	dcda(0.3*N - 1 + (N-1)*N) = penal*(65.0-0.2)*pow(a(0.3*N - 1 + N*(N-1)),penal-1)*dcdk(0.3*N - 1 + (N-1)*N);
	...
	dcda(0.7*N + (N-1)*N) = penal*(65.0-0.2)*pow(a(0.7*N + N*(N-1)),penal-1)*dcdk(0.7*N + (N-1)*N);
