...
fileID = fopen('temperature_0.txt','r');
formatSpec = '%f';
TC1 = fscanf(fileID,formatSpec);

fileID = fopen('temperature_0_N_50.txt','r');
formatSpec = '%f';
TC2 = fscanf(fileID,formatSpec);

fileID = fopen('temperature_0_N_100.txt','r');
formatSpec = '%f';
TC3 = fscanf(fileID,formatSpec);
...
pctmetal = 0.3*ones(N,N);
[k] = createK(pctmetal, N, penal);
[T3, K, f] = fvm_func(k, N, q);

n1 = norm(T1-TC1)/norm(T1);
n2 = norm(T2-TC2)/norm(T2);
n3 = norm(T3-TC3)/norm(T3);
...
%%
timingstot = sum(timings)/5;
timingscpp = [2207, 4544, 16637, 40880, 86598, 176944, 444373, 719001, 1770581, 3742271]*10^(-6);
figure()
hold on
plot(n,timingstot)
plot(n,timingscpp)
